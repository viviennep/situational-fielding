...
def calc_wall_properties(ofp,fences_df,verbose=False):
    ...
    hit_dist       = np.linalg.norm(land_pos,axis=-1)
    hit_vec_hat    = land_pos/hit_dist[:,None]
    dots           = np.einsum('ij,ij->i',diff_vec_hat,start_pos_hat)
    dets           = np.einsum('ij,ij,j->i',
                               diff_vec_hat,
                               start_pos_hat[:,::-1],
                               np.array((1,-1)))
    ...
    wall_hit_dist  = np.zeros_like(dists)
    wall_hit_h     = np.zeros_like(dists)
    ...
    for i,team in enumerate(team_l):
        ...
        park_segs = (fences_df.filter(cl('team_abbr').eq(team))
                              .select('fence_x','fence_y')
                              .to_numpy())
        park_hs   = (fences_df.filter(cl('team_abbr').eq(team))
                              .select('fence_height')
                              .to_numpy().squeeze())
        park_mask = team_i == i
        ds,heights,intersections  = calc_dist2wall(start_pos[park_mask], 
                                                   park_segs, 
                                                   start_pos_hat[park_mask], 
                                                   park_hs)
        wall_rad_dist[park_mask]  = ds
        ds,heights,intersections  = calc_dist2wall(start_pos[park_mask], 
                                                   park_segs, 
                                                   diff_vec_hat[park_mask], 
                                                   park_hs)
        wall_dir_dist[park_mask]  = ds
        wall_dir_h[park_mask]     = heights
        ds,heights,intersections  = calc_dist2wall(land_pos[park_mask], 
                                                   park_segs, 
                                                   diff_vec_hat[park_mask], 
                                                   park_hs)
        wall_ball_dist[park_mask] = ds
        ds,heights,intersections  = calc_dist2wall(land_pos[park_mask]*0,  # make all 0,0
                                                   park_segs, 
                                                   hit_vec_hat[park_mask], 
                                                   park_hs)
        wall_hit_dist[park_mask]  = ds
        wall_hit_h[park_mask]     = heights
        wall_min_dist[park_mask]  = np.linalg.norm((land_pos[park_mask,None] - park_segs),
                                                   axis=-1).min(-1)
    wall_dict = {'play_id': ofp.select('play_id').to_numpy().squeeze(),
                 'angle': angles, 
                 'dist': dists, 
                 'hit_dist': hit_dist,
                 'wall_dist_hit': wall_hit_dist,
                 'wall_height_hit': wall_hit_h,
